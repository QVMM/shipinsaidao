...
export function getOfflineVoiceStatus() {
  ...
  const ttsReady = naturalVoiceReady || fallbackVoiceReady
  ...
  return {
    ...
    nativeReady,
    nativeError,
    voice: naturalVoiceReady ? VOICE_NAME : FALLBACK_VOICE_NAME,
    voiceGender: 'female',
    ttsEngine: naturalVoiceReady ? 'zipvoice-local' : 'matcha-local',
    naturalVoiceReady,
    message: ready ? `离线识别与${naturalVoiceReady ? '自然' : '备用'}本地女声已就绪。` : `尚未安装或加载：${missing.join('、')}。`,
    modelDir: p.base,
  }
}
...
function getTts() {
  ...
  const kind = naturalReady && !preferMatcha ? 'zipvoice' : 'matcha'
...
export async function synthesizeOfflineVoice(text) {
  ...
  try {
    ...
    // Electron's hardened V8 runtime rejects addon-owned external ArrayBuffers.
    // Request a copied internal buffer so the same package works in Node and Electron.
    const audio = bundle.engine.generate({ text: say.slice(0, 300), generationConfig, enableExternalBuffer: false })
    const wav = encodeWaveBuffer(audio.samples, audio.sampleRate)
    ...
    return { ok: false, status: 500, error: `本地女声生成失败：${String(error?.message || error)}` }
  }
}
