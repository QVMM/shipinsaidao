...
async function stagedWebSmoke() {
  ...
  try {
    ...
    if (child.exitCode == null || existsSync(stateFile)) throw new Error('本地服务未在停止指令后完整退出。')
...
console.log(JSON.stringify({
  ok: true,
  platform: 'Windows 10/11 x64 browser',
  launch: '双击启动系统.bat',
  host: '127.0.0.1 only',
  nodeRuntime: 'Windows x86-64 已内置',
  voiceRuntime: 'Windows x86-64 已内置',
