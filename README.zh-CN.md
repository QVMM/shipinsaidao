...
交付电脑小白时，使用 `release/替抗蓟化-Windows本地Web版-1.0.10-universal.zip`。同一个包会自动适配 Intel/AMD x64 与 Qualcomm/Windows on ARM 电脑。用户只需先“全部解压”，再双击排在最上方的 `00-首次使用-安装并启动.bat`；程序会等到本地网页真正可访问后才打开浏览器。之后每次直接使用自动创建的“打开替抗蓟化网页”桌面图标。它会打开带地址栏的普通浏览器窗口，不是桌面应用；对方电脑不需要安装 Node.js、Python 或数据库。
